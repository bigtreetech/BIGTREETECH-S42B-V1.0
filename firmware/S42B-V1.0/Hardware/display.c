            ...
            case 0:                             //设置界面
                    switch(Menu_Num_item){      //菜单项选择
                        ...
                        case 1:OLED_ShowString(0,0,"  ");
                            OLED_ShowString(0,16,"->");
                        break ;
                        default:OLED_ShowString(0,0,"->");break ;//位置不变可以不写此句
                    }
                    if(Calibration_flag!=0xAA){                     //出产校准设置界面
                        OLED_ShowString(16,0,"Calibration");
                        OLED_ShowString(16,16,"Exit");
                    }
                    else if(Second_Menu==1){                                           //校准后功能设置界面
...
//                                break ;
                        ...
                        switch(Menu_Num2_item){
                                case 0: OLED_ShowString(16,0,"0   ");//驱动电流设置界面 
...
//                                                OLED_ShowString(16,48,"1400");
                    ...
                    else if(Second_Menu == 3){                      //电子齿轮（细分）选择
                        switch(Menu_Num3_item){
                            case 0: Microstep_Set=32;                 //待添加电子齿轮参数
...
//                                break;
                    ...
                    else if(Second_Menu == 4){                              //电机使能设置
                        switch(Menu_Num4_item){
                            case 0: //这里要理清逻辑，使能信号已主板的使能信号为准
//                                    if( enter2_once_flag==1){             //使能脚置1enmode==0 &&
 //                                       enter2_once_flag =0;
 //                                       enter1_once_flag=1;
//                                        if(enmode!=0)//1
                                            Dir_Enable= 0x55;
...
//                                    }
                        ...
                        switch(Menu_Num5_item){
                            case 0: if(dir2_once_flag ==1 ){
                                ...
                                break;
                            case 1: if(dir1_once_flag == 1){//初始默认是
                                        dir1_once_flag=0;
                                        dir2_once_flag=1;
                                        Dir_Enable =0x22;
                                    }
...
//                    }
            ...
            break;
            case 1:OLED_ShowString(0,2,"Simp:  0000 RPM");//主显示界面
...
/*************SIMP*****************************************************/
        ...
        Motor_speed=Motor_speed*60;//比较理想化
...
/*************Deg***************************************************/
        ...
        e_temp=e_temp*360*10/16384; //得到当前角度
        e_a =e_temp/10;                    //整数部分
        e_b =e_temp%10;                    //小数部分
        temp[13]=e_a/1000;
